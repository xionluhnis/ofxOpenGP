...
template <typename Scalar, int N>
inline Vector<Scalar,N> operator*(const Vector<Scalar,N>& v1, const Vector<Scalar,N>& v2)
{
...
public:
    ...
    Vector<Scalar,3> cross(const Vector<Scalar,3>& o) const
    {
    ...
    Scalar dot(const Vector<Scalar,3>& o) const {
        return opengp::dot(*this, o);
    }
...
template <typename Scalar>
inline Vector<Scalar,3> operator*(const Vector<Scalar,3>& v1, const Vector<Scalar,3>& v2)
{
    return Vector<Scalar,3>(v1.x*v2.x, v1.y*v2.y, v1.z*v2.z);
}
...
template <typename Scalar>
inline Scalar sqrdist(const Vector<Scalar,3>& v0, const Vector<Scalar,3>& v1)
{
    Scalar dist(0), d;
    for (int i=0; i<3; ++i)
    {
